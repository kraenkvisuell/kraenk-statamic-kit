...
- `Database/Seeders/TestUserSeeder` – a super user for testing, test@kraenk.de / password, seeded only when `APP_ENV` is local or staging; skipped when that email exists.
...
2. `php artisan migrate`, then `php artisan kit:init` (single- or multisite, demo pages and navigations, site settings, blog posts, projects and partners, SEO Pro site defaults, the super user test@kraenk.de / password).
